####################### DESCRIPTION #######################
###     Functions for the process partition model       ###
...
pp.inla.model = function(Q, log.prior) {
  ## Input
  # Q and log.prior
  ...
  ## Define th emodel
  model.rgeneric = inla.rgeneric.define(model = pp.rgeneric.model, Q = Q, log.prior = log.prior)
  return(model.rgeneric)
}

############################################################
### DEFINE THE MODEL SKELETON FUNCTION
pp.rgeneric.model = function(
...
pp.fem <- function(mesh, subdomain, sphere){
  ## input:
  ...
  for (i in 1:p){
    ...
    femi <- list(Cmat = Cmat, G1 = G1, G2 = G2, node_id = node_id)
    femList[[i]] <- femi
  }
  return(femList)
}
...
pp.precision <- function(femlist, theta){
  ...
  # - theta:  a p x 2 lenght vector for the parameters, p is the number of independent subsets
...
pp.create.Q = function(mesh, subdomain, sphere, initial.theta = NULL)
{
  ## Input:
  # - mesh: the entire mesh
  # - subdomain: list of triangle indices in each subdomain
  # - initial.theta: initial values for theta, can be null
  pp.precision = pp.precision
  p <- length(subdomain)
  
  if(is.null(initial.theta)) {initial.theta = rep(0, p*2)}
  
  femL <- pp.fem(mesh = mesh, subdomain = subdomain, sphere = sphere)
  
  ## Create Q function  
  Q <- function(theta){
    return(pp.precision(femlist = femL, theta = theta))
  }  
  return(Q)
}
...
pp.create.prior.log.norm = function(prior.param) {
...
############################################################
### DEFINE THE LOG.PRIOR FUNCTION
pp.create.priors2 = function(prior.param) {
