...
#########################################################################################################
## Try load from cache
cache_grace <- paste0(dd_root,"WP1-BHM/Experiment2/data/Exp2_JPL_grace.rda")
cache_alt <- paste0(dd_root,"WP1-BHM/Experiment2/data/Exp2_alt.rda")

## GRACE
if(file.exists(cache_grace)){
  load(cache_grace)
}else{
  source(paste0(wd_root, "Experiment2/Rscript/preprocessing/GRACE_JPL_data.R")) 
