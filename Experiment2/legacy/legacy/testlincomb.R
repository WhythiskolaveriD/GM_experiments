...
## Suppose we want to find the marginal std of Y = AX, then V(Y) = A %*% Sigmax %*% t(A)
## Suppose is a row standardised matrix that maps the 2500 Xs to 3 Ys
A <- matrix(rbinom(7500, 1, 0.5), nrow = 3, ncol = 2500)
A <- A/rowSums(A)
Vy <- A %*% Matern_cov %*% t(A)
Vy2 <- A %*% Diagonal(x=diag(Matern_cov)) %*% t(A)
